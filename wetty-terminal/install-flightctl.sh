...
# Method 1: Extract from cli-artifacts pod (most reliable)
CLI_POD=$(oc get pods -n "$EM_NS" -o name 2>/dev/null | grep cli-artifacts | head -1)
if [ -n "$CLI_POD" ]; then
  POD_NAME="${CLI_POD#pod/}"
  oc exec -n "$EM_NS" "$POD_NAME" -- \
    cat /home/server/src/gh-archives/amd64/linux/flightctl-linux-amd64.tar.gz \
    > /tmp/flightctl.tar.gz 2>/dev/null
  if [ -s /tmp/flightctl.tar.gz ]; then
...
# Method 2: Download from cli-artifacts route (fallback)
CLI_ROUTE=$(oc get route -n "$EM_NS" flightctl-cli-artifacts-route -o jsonpath='{.spec.host}' 2>/dev/null)
if [ -n "$CLI_ROUTE" ]; then
  wget -q --no-check-certificate \
    "https://${CLI_ROUTE}/linux/amd64/flightctl-linux-amd64.tar.gz" \
